...
export function mergeSingularityServerSnapshotRunners(existingRunners = [], snapshotPayload = {}, options = {}) {
  ...
  const runners = participants.map((participant, index) => {
    ...
    return Object.freeze({
      ...
      effectKind: typeof participant.effectKind === "string" ? participant.effectKind : "", sizeScale: finiteNumber(participant.sizeScale, 1) > 0 ? finiteNumber(participant.sizeScale, 1) : 1,
      serverAttackUntilMs: Math.max(finiteNumber(existing?.serverAttackUntilMs, 0), 0), serverAttackVisualId: Math.max(finiteNumber(existing?.serverAttackVisualId, 0), 0), serverAttackFacingDirection: existing?.serverAttackFacingDirection || "", serverSkillUntilMs: Math.max(finiteNumber(existing?.serverSkillUntilMs, 0), 0),
      serverOwned: true,
      serverProgress,
      serverLaneOffsetPx,
