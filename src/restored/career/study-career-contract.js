...
export const RESTORED_STUDY_ACTIVITY_CATALOG = Object.freeze([
  Object.freeze({ id: RESTORED_STUDY_ACTIVITY_IDS.LIBRARY_SELF_STUDY, placeId: "baegeum:library", label: "Library Self Study", costDp: 0, minutes: 120, creditGain: 4, statDeltas: Object.freeze({ intelligence: 4, energy: -8, mental: -2 }) }),
  Object.freeze({ id: RESTORED_STUDY_ACTIVITY_IDS.UNIVERSITY_NIGHT_CLASS, placeId: "baegeum:university", label: "University Night Class", costDp: 25000, minutes: 180, creditGain: 10, statDeltas: Object.freeze({ intelligence: 7, energy: -12, mental: -5 }) })
]);

export const RESTORED_COMPANY_LEVELS = Object.freeze([
  Object.freeze({ id: "office_trainee", title: "Company Trainee", requiredCredits: 8, requiredIntelligence: 48, baseWageDp: 65000, promotionThreshold: 12, nextLevelId: "office_staff" }),
  Object.freeze({ id: "office_staff", title: "Office Staff", requiredCredits: 20, requiredIntelligence: 56, baseWageDp: 92000, promotionThreshold: 28, nextLevelId: "assistant_manager" }),
  Object.freeze({ id: "assistant_manager", title: "Assistant Manager", requiredCredits: 42, requiredIntelligence: 68, baseWageDp: 135000, promotionThreshold: null, nextLevelId: null })
]);
...
export function createRestoredStudyResult(activityId, state = {}, options = {}) {
  ...
  if (activity.costDp > cash) return Object.freeze({ ok: false, reason: "not_enough_cash", message: "수강료가 부족합니다." });
  const effects = [];
  if (activity.costDp > 0) effects.push(effect(RESTORED_STUDY_CAREER_EFFECT_TYPES.ECONOMY_LEDGER_ENTRY, { entryType: "study_tuition", currency: "DP", deltas: { cash: -activity.costDp }, placeId: activity.placeId }));
...
export function createRestoredCompanyShiftResult(state = {}, performance = {}) {
  ...
  const wageDp = Math.round(level.level.baseWageDp * (score >= 85 ? 1.2 : score >= 65 ? 1 : 0.8));
  ...
  return Object.freeze({
    ...
    effects: Object.freeze([
      effect(RESTORED_STUDY_CAREER_EFFECT_TYPES.ECONOMY_LEDGER_ENTRY, { entryType: "company_wage", currency: "DP", deltas: { cash: wageDp }, placeId: "baegeum:company-district" }),
      ...
      effect(RESTORED_STUDY_CAREER_EFFECT_TYPES.CAREER_STATE_PATCH, { trackId: RESTORED_COMPANY_TRACK_IDS.BAEGEUM_OFFICE, levelId: next.levelId, promotionPoints: next.promotionPoints, totalCompanyIncome: wageDp, completedCompanyShifts: 1 }),
      effect(RESTORED_STUDY_CAREER_EFFECT_TYPES.UI_MESSAGE, { message: next.promoted ? `승급: ${getRestoredCompanyLevel(next.levelId)?.title}` : `${level.level.title}: ${formatDp(wageDp)}` })
    ]),
    message: next.promoted ? `회사 승급: ${getRestoredCompanyLevel(next.levelId)?.title}` : `회사 근무 완료: ${formatDp(wageDp)}`
  });
}
...
export function validateRestoredStudyCareerContract() {
  ...
  if (!library.ok || library.effects.some((item) => item.payload?.deltas?.cash)) errors.push("library study must not cost DP");
  if (!university.ok || !university.effects.some((item) => item.payload?.deltas?.cash < 0)) errors.push("university study must cost DP");
  if (blocked.ok || blocked.reason !== "not_qualified") errors.push("company work must be gated by study");
  if (!company.ok || company.wageDp <= 0 || !company.promoted) errors.push("qualified company work must pay and promote");
