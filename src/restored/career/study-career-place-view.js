...
const SUPPORTED_ACTIONS = Object.freeze(["job_places", "library", "university", "company"]);
...
export function renderRestoredStudyCareerPlaceHtml(actionId, state = {}) {
  ...
  const companyButtons = listRestoredCompanyShiftPresets().map((preset) => {
    ...
    const detail = result.ok
      ? `${preset.detail} · ${result.workedLevelTitle || result.levelTitle} · ${result.wageWon.toLocaleString("ko-KR")}원${result.promoted ? " · 승급 예정" : ""}`
      : result.message;
    ...
    <section id="study-career-panel" class="col-span-1 sm:col-span-2 rounded-2xl border border-blue-100 bg-white p-4 shadow-sm">
      <div class="flex items-start justify-between gap-3">
        <div>
          ...
          <p class="mt-1 text-xs font-bold text-slate-500">공부로 지능과 학점을 올리고, 회사 근무 방식에 따라 원화 수입과 승급 포인트를 얻습니다.</p>
        </div>
      ...
      <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
        ...
        ${button("대학 야간강의", "25,000원 · 학점 +10", "completeStudyCareerAction('study:university-night-class')", "indigo", university.ok, university.message)}
      </div>
...
export function validateRestoredStudyCareerPlaceView() {
  ...
  const lockedHtml = renderRestoredStudyCareerPlaceHtml("job_places", {});
  const libraryHtml = renderRestoredStudyCareerPlaceHtml("library", readyState);
  const companyHtml = renderRestoredStudyCareerPlaceHtml("company", readyState);
  if (lockedHtml.includes('onclick="showToast("')) errors.push("locked company buttons must not break onclick attributes with nested double quotes");
  if (!lockedHtml.includes('onclick="showToast(\'')) errors.push("locked company buttons must keep a safe toast action");
  if (!libraryHtml.includes("study:library-self-study")) errors.push("library building must expose study action");
  if (!companyHtml.includes("career:company-shift:documents")) errors.push("company building must expose company work");
  for (const hook of ["study:library-self-study", "study:university-night-class", "career:company-shift:documents"]) {
    if (!html.includes(hook)) errors.push(`study/career view must expose ${hook}`);
  }
...
function button(label, detail, onclick, tone, enabled, disabledMessage = "") {
  ...
  const click = enabled ? onclick : `showToast('${escapeJsSingleQuotedString(disabledMessage || "조건이 부족합니다.")}')`;
