...
- `src/restored/games/marathon-input-contract.js`, `marathon-character-skill-contract.js`, and `marathon-combat-contract.js` own the first Singularity Race action layer: WASD/mobile movement with no player sprint toggle, E skills, mouse attack stall, checkpoint meme-style character rewards, and checkpoint respawn.
