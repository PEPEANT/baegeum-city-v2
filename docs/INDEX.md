...
- [Simulacra World Common Engine Plan](plans/simulacra-world-engine.md): Common-engine branch plan for shared modules, Singularity Race as the first active derived game, Drawing World as the second candidate, and Iron Line as an ops/admin reference only.
...
- [Singularity Race Cloudflare Online](singularity-race-cloudflare-online.md): First public online slice for one Cloudflare Workers + Durable Objects room with 50-runner cap, 10 Hz input, 10 Hz snapshots, chat limits, and no common-engine expansion.
