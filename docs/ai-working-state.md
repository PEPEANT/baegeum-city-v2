...
Next: Run full `npm run check`, then commit/push/deploy only after the user asks. Public live testing should start by pressing admin `방 만들기`, then user queue join, then admin `인게임 입장 시작`, then admin `시작`.
