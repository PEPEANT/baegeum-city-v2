...
Verified: `node tools/smoke-index-entry.cjs`, `node tools/smoke-singularity-race-v01-lock.cjs`, `node tools/check-restored-ui-online-ranking-chat-roadmap.cjs`, `git diff --check`, and full `npm run check` passed. Browser fallback on `http://127.0.0.1:4173/index.html?codex=event-launcher-v3` at 390px confirmed title/H1 `특이점레이스`, primary `이벤트 참가 공개방` Cloudflare href, no `onlineRaceLink`, no `.hub-strip/.hub-chip`, category menu closed by default, menu opens with login/skin/shop/archive slots, login modal opens from the menu, and no horizontal overflow before or after opening the menu.
Blocked: The in-app Browser `iab` target was unavailable, so browser verification used the available Playwright fallback. `singularity-race-admin.html` is currently dirty from a separate token-storage/admin-page change and was not part of this launcher edit.
Next: Decide whether to commit/deploy only the launcher change or also include the separate admin-token page change if that was intentional.
