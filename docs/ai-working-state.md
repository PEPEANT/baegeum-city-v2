...
Verified: `node --check src/restored/online/singularity-race-cloudflare-client.js`, `node tools/smoke-singularity-race-cloudflare-online.cjs`, `npm run check:singularity-race`, and `git diff --check` passed. Live checks before the fix confirmed the preview root is a launcher, the redirected player route preserves query params, `/singularity-race` contains the new room-directory code, and the Worker `/rooms?includeUserRooms=1` response was healthy but had `activeUserRooms:0`.
Next: Run full `npm run check`, then commit/push and redeploy the Cloudflare Pages preview/current client so direct public player links show the user-room directory without requiring query parameters.
