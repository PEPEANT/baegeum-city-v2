...
Next: Commit/push/deploy the accumulated room-name, online narration, and room-directory fixes together when ready.
Do not: Re-couple `includeUserRooms=1` to the currently selected room id, or show `room:singularity-race:public-001` as a normal player room card.

Date: 2026-06-04
Observed: User chose the real online narration path so the admin page narration button should reach other devices, not only same-browser dev tabs.
Changed: Added an authenticated Cloudflare Worker `/admin/narration` endpoint. It only succeeds before countdown/racing when the public room is active, entry is open, and at least one player exists, then broadcasts `narration_start` with the selected script id. The player page now consumes that packet, enters Cloudflare race staging if needed, and reuses the existing typewriter narration overlay. The admin page now has a `나레이션 재생` button beside start; in Cloudflare mode it calls `/admin/narration`, and in dev mode it keeps using the room-scoped narration localStorage/BroadcastChannel rehearsal. Updated Cloudflare online docs, the marathon plan, and smokes.
Verified: `node --check workers/singularity-race-worker.js`, `node tools/smoke-singularity-race-narration.cjs`, `node tools/smoke-singularity-race-cloudflare-worker-contract.cjs`, `node tools/smoke-singularity-race-cloudflare-online.cjs`, `node tools/smoke-singularity-race-admin-observer.cjs`, `node tools/check-size.cjs`, `git diff --check`, `npm run check:singularity-race`, and full `npm run check` passed. Browser verification reached the admin page but was blocked by the admin-token prompt modal because the available Playwright tool cannot handle that dialog in this session.
Next: Commit/push/deploy the room-name input and online narration changes together if the user wants them live.
Do not: Send narration through public player-owned packets, or let narration affect movement, ranking, item, reward, finish, or other authority state.

Date: 2026-06-04
Observed: User narrowed the current user-room request to only allowing the room creator to choose a room name.
Changed: Added a room-name input next to the Cloudflare user-room create button in the Singularity Race lobby. Creating a user room now sends the typed `displayName` to `POST /rooms/create`; blank input falls back to the creator nickname-based default. The create row stays hidden once connected, disables during room fetches, and stacks cleanly on mobile. Updated the Cloudflare online smoke and docs to guard the room-name request path.
Verified: `node tools/smoke-singularity-race-cloudflare-online.cjs`, `npm run check:singularity-race`, `node tools/check-size.cjs`, `git diff --check`, and full `npm run check` passed. Browser verification was partially limited because the in-app Browser `iab` was unavailable and the fallback Playwright session closed during the create-click check; the fallback did confirm the local page contains the new input/button DOM and the automated smoke guards the request body token.
Next: Commit/push/deploy this client-only room-name input change if the user wants it live.
Do not: Expand this into broader lobby/room-discovery changes unless the user asks again.

Date: 2026-06-04
Observed: User confirmed the basic punch range problem: client and Worker were measuring reach in progress percent, so the visible hit distance changed by map length/route shape and could feel like long-range hits.
Changed: Added `src/restored/games/singularity-race-basic-attack-range.js` as the shared client/Worker basic attack range contract. Basic attacks now convert runners to trail-world pixels and resolve a fixed capsule with 118px forward reach, 54px radius, and 18px rear grace. `singularity-race.html` uses the shared helper for local target selection/hit results, and `workers/singularity-race-worker.js` uses the same helper for server-owned public `attack_action` stun selection instead of `BASIC_ATTACK_RANGE_PROGRESS`. Updated reverse/Worker smokes and docs to reflect the shorter visible-distance hitbox.
Verified: `node --check workers/singularity-race-worker.js`, `node --check src/restored/games/singularity-race-basic-attack-range.js`, `node tools/check-restored-marathon-contract.cjs`, `node tools/smoke-singularity-race-reverse-attack.cjs`, `node tools/smoke-singularity-race-cloudflare-worker-contract.cjs`, `node tools/smoke-singularity-race-cloudflare-online.cjs`, `node tools/check-size.cjs`, `npm run check:singularity-race`, `git diff --check`, and full `npm run check` passed. The in-app Browser `iab` target was unavailable, so fallback Playwright opened local `singularity-race.html?devOnline=1&adminLaunch=1&roomId=room:singularity-race:dev-001&t=basic-attack-px`; the page reached `race`, loaded one module script, and reported no console error/warning/pageerror messages.
Next: If live feel still reads too short/long, tune only the shared pixel constants in `singularity-race-basic-attack-range.js`; do not reintroduce progress-percent basic attack range checks.
Do not: Use client-sent attack origin/position for public hit authority or restore `BASIC_ATTACK_RANGE_PROGRESS` for basic punches.

Date: 2026-06-04
Observed: User reported that the room-host control popup still felt bad on mobile/PC: after collapsing it, a blurred/mosaic-like floating layer remained, buttons were awkward, and the public room card `특이점레이스 공개방 001` should stay out of the way while user rooms are the main flow.
Changed: Moved the Cloudflare user-room host controls out of the lobby room-panel overlay and into the queue action area. Queue now shows host open/start/cancel inline below the runner slots; race staging shows only a compact top-center start button; countdown/racing hide host controls from the track. Removed the old collapsed host popup styling, hid the host toggle, and stopped reviving `.room-panel` as a fixed queue/race overlay. The room directory now filters the closed public room card out whenever active user rooms exist, so user-created rooms are the visible lobby focus.
Verified: `node tools/smoke-singularity-race-cloudflare-online.cjs`, `node tools/check-size.cjs`, `npm run check:singularity-race`, full `npm run check`, and `git diff --check` passed. Browser layout checks on local `singularity-race.html` confirmed desktop queue host controls live inside `.queue-actions` with no backdrop blur and hidden `.room-panel`; desktop race shows only a 260x44 top-center start button; mobile queue stacks the host buttons vertically; mobile race shows only a 210x44 top-center start button.
Next: If the user likes the local behavior, commit/push/deploy this client-only UI change. If not, tune exact mobile spacing before deploy.
Do not: Restore `.shell.cloudflare-host-active[data-screen="queue|race"] .room-panel` fixed overlay rules or the old collapsible host popup styling.

Date: 2026-06-04
Observed: User reported that rooms still did not appear from the Cloudflare Pages preview root link `https://39826afc.singularity-race-client.pages.dev`. Live Worker inspection showed the room directory API was deployed and healthy, but there were zero active user rooms at that moment; the remaining client-side trap was that direct public player URLs such as `/singularity-race` without `?online=cloudflare&serverUrl=...` loaded the latest page code while leaving Cloudflare online mode disabled.
Changed: Public deployment hosts now default the Singularity Race player page to the public Cloudflare Worker. `src/restored/online/singularity-race-cloudflare-client.js` exports the public Worker WS endpoint and recognizes `simulacra-world.vercel.app`, `singularity-race-client.pages.dev`, and Cloudflare Pages preview subdomains. `singularity-race.html` turns Cloudflare online on automatically for those hosts unless `offline=1`, `online=local`, or `devOnline=1` is present. The Cloudflare online smoke now dynamically verifies that the preview host resolves to `wss://singularity-race-online.rneetn.workers.dev/ws`.
Verified: `node --check src/restored/online/singularity-race-cloudflare-client.js`, `node tools/smoke-singularity-race-cloudflare-online.cjs`, `npm run check:singularity-race`, full `npm run check`, `git diff --check`, and browser verification on the deployed no-query preview route passed. Live checks before the fix confirmed the preview root is a launcher, the redirected player route preserves query params, `/singularity-race` contains the new room-directory code, and the Worker `/rooms?includeUserRooms=1` response was healthy but had `activeUserRooms:0`. After deploy, `https://c320d181.singularity-race-client.pages.dev/singularity-race` resolved the no-query player page to `wss://singularity-race-online.rneetn.workers.dev/ws` with visible Cloudflare room controls; the old `https://39826afc.singularity-race-client.pages.dev/singularity-race` preview remained stale and still resolved to its own `/ws`.
Deployed: Committed `8fa2e51 Default race preview links to Cloudflare online`, pushed `main`, and deployed Cloudflare Pages project `singularity-race-client`; new preview URL is `https://c320d181.singularity-race-client.pages.dev`.
Next: Share the canonical/current player URL or the new preview URL instead of stale deployment-hash URLs like `39826afc`.
Do not: Revert the explicit query-param entry links; they remain valid and useful for custom Worker endpoints.
...
Next: Commit/push/deploy the host-control collapsed-title hotfix.
Do not: Leave `방장 컨트롤` or the room code visible while the host controls are collapsed.
...
Changed: Added a `접기`/`열기` toggle to the Cloudflare host controls. The host overlay hides the title, room code, and `입장 열기`/`시작`/`방 취소|방 종료` action row when collapsed, preserves the collapsed state during the current page session, and keeps ARIA expanded state in sync. The online smoke guards the toggle button, collapsed state, helper, hidden action-row CSS, and hidden collapsed header-text CSS.
Verified: `node tools/smoke-singularity-race-cloudflare-online.cjs`, `npm run check:singularity-race`, `git diff --check`, and full `npm run check` passed. Browser verification on local `singularity-race.html` confirmed the host controls toggle from `접기` to `열기`, hide actions and header text while collapsed, shrink to the reopen toggle, and reopen without page errors.
Next: Commit/push/deploy the client-only host-control toggle if the user wants it live.
Do not: Remove the fixed queue/race host overlay, or leave the room code/title visible while collapsed.
...
Deployed: Committed `076ada7 Allow race hosts to cancel rooms`, pushed `main` to `origin/main`, deployed Cloudflare Worker `singularity-race-online` version `607d5683-4bd2-4fa4-883e-8ef61f2ef819`, and verified production `https://simulacra-world.vercel.app/singularity-race.html` contains the host cancel label, confirmation copy, and closed-room host-token cleanup hook. Live Worker smoke created temporary user room `WYGG2H`, then `/rooms/host/end` before final results returned `roomActive:false`, `roomStatus:"closed"`, and no `resultSnapshot`.
Next: Run the two-person live user-room flow end to end: create room, host auto-join, open entry, second player join, start, finish window, results, save/share, and host cancel/close recovery checks.
Do not: Reintroduce `result_not_finalized` as a host-end blocker, or create client-calculated rankings for canceled rooms.
...
Next: Human-play or live-test a 2+ runner Cloudflare room and compare ping/snapshot smoothness after this CPU fix before considering spectator Hz split, delta snapshots, or further attack range tuning.
Do not: Build delta snapshots, per-player interest culling, or another broad network protocol layer until remaining lag is measured after this one-pass broadcast fix.

Date: 2026-06-04
Observed: User asked whether the planned Singularity Race user-room result-card and full room shutdown/auto-delete flow was actually being built, after the first user-room create/host-control slice.
Changed: Added server-owned user-room result lifecycle on top of the Cloudflare Durable Object room model. The Worker now creates `resultSnapshot` once when the 30 second finish window expires, includes it in `race_finished`, summaries, and closed-room payloads, moves user rooms through `open` -> `results_finalized` -> `closed` -> `deleted`, blocks host room end until results are finalized, clears host tokens on close, rejects joins/chat/commands for closed/deleted rooms, and auto-closes/deletes user rooms by TTL. `/rooms/create` now routes through the fixed public-room registry path so creation has a 60s client/IP cooldown, a 12 active user-room cap, and stale-room cleanup before issuing a new short code. The player page now keeps Cloudflare `resultSnapshot`, renders the existing result panel from it, keeps finalized results visible when a room closes, and adds result PNG save, Web Share, and text-copy fallback buttons.
Verified: `node --check workers/singularity-race-worker.js`, `node --check src/restored/online/singularity-race-cloudflare-client.js`, `node --check tools/smoke-singularity-race-cloudflare-host-contract.cjs`, `node --check tools/smoke-singularity-race-cloudflare-worker-contract.cjs`, `node tools/smoke-singularity-race-cloudflare-host-contract.cjs`, `node tools/smoke-singularity-race-cloudflare-worker-contract.cjs`, `node tools/smoke-singularity-race-cloudflare-online.cjs`, `npm run check:singularity-race`, `node tools/check-size.cjs`, `git diff --check`, and full `npm run check` passed. Local browser fallback on `http://127.0.0.1:4173/singularity-race.html?resetProfile=1` loaded without page errors and confirmed the new result save/share/copy buttons exist and are hidden before a result snapshot.
Blocked: Live Worker/browser end-to-end create/open/join/start/finish/close verification was not run in this slice; deploy is still needed for `simulacra-world.vercel.app` and the public Worker to receive the new behavior.
Next: If the user wants this live, commit/push/deploy both client and Worker, then human-play one short-code room through finish-window expiry and host close; verify result card save/share/copy from the closed room.
Do not: Let clients recalculate final rankings at close, let host tokens survive room close, or allow unlimited short-code room creation without cooldown/cap guards.

Date: 2026-06-04
Observed: User clarified the deeper motive behind the project: the game was partly a way to be recognized. AI validation does not fill that need, so the next direction may be a separate text-first novel/comic/world-story about a life endlessly trying to be recognized, rather than forcing the feeling into monetization, online scope, or another large game feature.
Changed: Extended `docs/creator-context-archive-note.md` with a recognition-turn section and the decision signal to back up R1 while starting the recognition story elsewhere as text. Created a separate text-only starter folder at `../endless-recognition-novel` with `README.md` and `00-world-seed.md`.
Verified: `git diff --check` passed, and full `npm run check` passed. Usual warning remains: `tools/check-restored-life-job-contract.cjs` longest function is 102 lines.
Next: Create a backup commit/remote branch for the current R1 state, then start a separate text-only folder for the new story seed.
Do not: Treat the novel direction as approval to expand the R1 game, add monetization, deploy, or build new online systems.

Date: 2026-06-04
Observed: User clarified that the proposed Markdown record is not meant to pivot the product or change the current implementation plan. It is meant as a creator-context archive for future AI/AGI readers so they can understand the human pressure behind R1/Baegeum City/Simulacra World: monetization anxiety, fear of future AI making better games, online scope being both the fun source and the danger, and the urge to preserve the creator's 고민 rather than immediately build another large feature.
Changed: Added `docs/creator-context-archive-note.md` and linked it from `docs/INDEX.md`. The note explicitly says it is context, not a work queue or approval to add monetization, Play Store publishing, online backends, phone community features, cross-game identity linking, or Chicken Job Online implementation.
Verified: `git diff --check -- docs/creator-context-archive-note.md docs/INDEX.md docs/ai-working-state.md` passed, and full `npm run check` passed. Usual warning remains: `tools/check-restored-life-job-contract.cjs` longest function is 102 lines.
Next: Treat the new note as interpretive context for future sessions. If product work resumes, ask whether the human wants documentation, a smallest playable slice, or continuation of the existing Singularity Race/Baegeum City work queue.
Do not: Convert the creator-context archive into scope expansion, cash-shop work, public publishing work, or a new online-game implementation without explicit human direction.

Date: 2026-06-03
Observed: User wants Singularity Race to support user-created rooms: a visible create-room button, random short room code, and room-scoped host controls for opening entry, starting, and ending only that room.
Changed: Added dynamic Cloudflare Durable Object room routing on top of the existing `SingularityRaceRoom` class. `/rooms/create` now creates `room:singularity-race:user:<CODE>` with a random six-character code and host token, `/rooms?roomId=...` reads the selected room, and `/rooms/host/open|start|end` require that room's `X-Host-Token`. The player page now shows a Cloudflare `방 만들기` button, stores the host token in tab-scoped `sessionStorage`, switches the current room/URL to the created room, exposes `입장 열기`, `시작`, and `방 종료` host controls, and includes the host token on WebSocket join so the host participant is marked.
Verified: `node --check workers/singularity-race-worker.js`, `node --check src/restored/online/singularity-race-cloudflare-client.js`, `node --check tools/smoke-singularity-race-cloudflare-worker-contract.cjs`, `node --check tools/smoke-singularity-race-cloudflare-online.cjs`, `node tools/smoke-singularity-race-cloudflare-worker-contract.cjs`, `node tools/smoke-singularity-race-cloudflare-host-contract.cjs`, `node tools/smoke-singularity-race-cloudflare-online.cjs`, `npm run check:singularity-race`, `node tools/check-size.cjs`, `git diff --check`, and full `npm run check` passed. Headless browser verification on `http://127.0.0.1:4173/singularity-race.html?online=cloudflare&serverUrl=ws://127.0.0.1:8787/ws` reached the lobby, showed a visible enabled `방 만들기` button, and kept the host controls hidden without a host token. Live Worker create/open/start/end verification is still pending for this slice.
Next: Test against a running/local or live Worker to create a real short-code room, open entry, join at least one runner, start, and end the room, then deploy the Worker and client if the user wants the feature live.
Do not: Give user-room hosts global admin authority, store host tokens in localStorage, or let player `start_request` bypass the host/admin HTTP controls.

Date: 2026-06-03
Observed: User reported Singularity Race runners getting hit by nothing and suspected the basic attack range/hitbox might be too wide. Inspection showed the public Worker already resolves `attack_action` hits and publishes stun/slow snapshots, but the client did not render server-owned attack/skill packets before the snapshot, making legitimate remote hits look invisible. The basic attack range was also broad at 4.8 progress with a 132px lane prefilter and 70-degree Worker arc.
Changed: Tightened basic attack tuning to 3.2 progress, 96px client lane prefilter, 52px lane-to-progress projection, and 58-degree Worker arc. The public client now consumes server-owned `attack_action` and `skill_use` packets, marks the attacker with a short one-shot attack visual, applies the existing target impact/stun cue immediately, resolves the local connected participant id correctly, and preserves short server attack visual metadata through immediate snapshot merges. Split the user-room host endpoint smoke into its own file so the existing Worker contract smoke stays under the project line limit. Updated Cloudflare online and stadium docs plus smoke guards.
Verified: `node --check workers/singularity-race-worker.js`, `node --check src/restored/games/singularity-race-dev-online.js`, `node tools/smoke-singularity-race-cloudflare-worker-contract.cjs`, `node tools/smoke-singularity-race-cloudflare-host-contract.cjs`, `node tools/smoke-singularity-race-reverse-attack.cjs`, `node tools/smoke-singularity-race-render-budget.cjs`, `node tools/smoke-singularity-race-cloudflare-online.cjs`, `node tools/smoke-singularity-race-cloudflare-worker-skill.cjs`, `node tools/smoke-singularity-race-combat-full-race.cjs`, `npm run check:singularity-race`, full `npm run check`, and `git diff --check` passed.
Blocked: Browser/live public-room verification was not repeated in this slice.
Next: Human-play one public or local connected race with two nearby runners; confirm hits now show the attacker/impact and that basic attacks require visibly close spacing.
Do not: Re-expand base attack to 4.8 progress/132px lane unless the visual range is deliberately redesigned.

Date: 2026-06-03
Observed: User wanted Singularity Race to remove the mobile run/sprint button and PC Shift sprint, then compensate by making the base running speed slightly faster.
Changed: Removed the visible mobile sprint control by keeping `#race-sprint-button` hidden/aria-hidden and deleting its mobile sizing rule. Player Shift input is ignored for race speed, `setVirtualSprint()` only clears stale Shift state, local sprint animation no longer activates from Shift, and the input contract keeps Shift-held movement on base `run`/`push`. Retuned local prediction and Worker movement to one base pace: `0.64%/s` progress and `134px/s` lane, with legacy sprint packets normalized to the same values. Updated Singularity Race, Cloudflare, restored README, stadium-plan wording, and smoke guards so future work does not restore sprint speed lift.
Verified: `node tools/smoke-singularity-race-progression.cjs` reported 150 seconds for both run and Shift-held movement, with 134/134 lane speeds. `node tools/smoke-singularity-race-mobile-race-ui.cjs`, `node tools/smoke-singularity-race-render-budget.cjs`, `node tools/smoke-singularity-race-cloudflare-online.cjs`, `node tools/check-restored-marathon-contract.cjs`, `node tools/check-size.cjs`, `npm run check:singularity-race`, full `npm run check`, and `git diff --check` passed. Headless Chrome verification at `390x740` loaded the page complete with no log errors and confirmed the sprint button exists only as hidden compatibility DOM (`hidden`, `aria-hidden`, `tabindex="-1"`, `display:none`, no visible box).
Deployed: Committed `a06031b Ship race pacing and launcher cleanup` and pushed `main` to `origin/main`. Deployed Cloudflare Worker `singularity-race-online` version `847c13b5-5b62-4310-95c2-8823e7250f4b`. Direct Vercel CLI deploy was blocked by missing local Vercel credentials, but the GitHub push-triggered production deployment was live-verified at `https://simulacra-world.vercel.app/`: latest race HTML includes `0.64` base speed, hidden sprint button, wall boundary clip import, and Drawing World API proxy; `/api/drawing-world-rooms` and the new boundary clip module both returned 200.
Blocked: None for the deployed code. The public Worker room is currently inactive/closed (`roomActive:false`, `entryOpen:false`) until the admin creates/opens the next round.
Next: Human-play one local/public race to judge whether `0.64%/s` and `134px/s` feel just slightly faster or need a tiny second tune.
Do not: Reintroduce a visible mobile sprint button, make Shift produce `mode:"sprint"` from player input, or let Worker `pace:"sprint"` move faster than base run.

Date: 2026-06-03
Observed: User suspected that an old wall collision still remained around a bent Singularity Race section where the gray wall/loop artifact had previously been visually removed. Code inspection confirmed the visual wall clip and the lane-boundary collision feedback were separate: wall SVG paths were clipped at folded/interior runs, but local movement still turned every lane-boundary touch into `collisionAtMs` and a boundary cue.
Changed: Added `src/restored/games/marathon-trail-boundary-clip.js` and exported lane-boundary clip visibility through `marathon-trail-geometry.js`. `singularity-race.html` now suppresses the local invisible-wall collision flash/cue when the touched boundary side is in a visually clipped wall segment, while keeping the lane clamp itself and normal visible-wall boundary feedback. `singularity-race-prediction.js` uses the same visibility rule, and geometry/prediction contracts guard the folded-bend case.
Verified: Browser fallback loaded `http://127.0.0.1:4173/singularity-race.html?debug=1` with no console/page errors, and in-page module evaluation returned `clippedBend:true`, `visibleWall:false`, `oppositeSide:false`. `node tools/check-restored-marathon-contract.cjs`, `npm run check:singularity-race`, `node tools/check-size.cjs`, `git diff --check`, and full `npm run check` passed. Usual warning remains: `tools/check-restored-life-job-contract.cjs` longest function is 102 lines.
Next: Human-play the folded bend in the basic map; runners should still stay on-course, but the removed gray wall segment should no longer produce an invisible boundary bump/cue.
Do not: Remove the road lane clamp entirely or allow folded-bend shortcuts while fixing the wall artifact feedback.

Date: 2026-06-03
Observed: User confirmed the race finish window should be a clean fixed 30 seconds and reported that the countdown still did not start when the first-place runner entered the finish. Root cause was Worker `refreshPhase()`: when all current players were finished, including a solo test room, `allFinished` skipped `finish_window_started` and moved straight to `finished`. The finish-window contract was also still the old 45-90 second variable formula.
Changed: `src/restored/games/singularity-race-finish-window.js` now resolves every finish window to fixed 30 seconds. `workers/singularity-race-worker.js` now starts `finish_window_started` for any first finisher, even when all current players are already finished, includes `finishWindowStartedAtMs` and `finishWindowDurationMs` in the packet, and only finalizes the race when the finish window expires. Updated finish-window, Cloudflare online, and Worker contract smokes to guard solo/all-finished countdown behavior. Updated Cloudflare and stadium docs to the fixed 30 second rule.
Verified: `node tools/smoke-singularity-race-finish-window.cjs`, `node tools/smoke-singularity-race-cloudflare-worker-contract.cjs`, `node tools/smoke-singularity-race-cloudflare-online.cjs`, `npm run check:singularity-race`, `node tools/check-size.cjs`, and full `npm run check` passed.
Blocked: Browser/live online verification was not repeated after this Worker change. Public deployment needs the Worker redeployed as well as the client, otherwise the live room may still use the old finish behavior.
Next: In online test, finish once in a solo room and once in a multi-player room; both should show a visible 00:30 finish countdown before final results.
Do not: Restore all-finished immediate finalization or the 45-90 second variable finish-window formula.

Date: 2026-06-03
Observed: User wanted the launcher cleaned so old Baegeum City v1 and Dice City v1 entries are hidden, while the visible secondary products become `배금도시 온라인` and `드로잉월드 온라인`. User also asked for a Drawing World online user count like `0/`.
Changed: Updated `index.html` so the secondary mode grid now has only `배금도시 온라인` and `드로잉월드 온라인`; the old Dice City v1 and Baegeum City v1 launcher cards are no longer linked. Added a Drawing World online count pill and a same-origin Vercel API proxy at `api/drawing-world-rooms.js` that reads `https://drawing-world.onrender.com/api/rooms` without storing secrets. Updated launcher and Singularity Race lock smokes to guard the two-card launcher and count endpoint.
Verified: The Drawing World proxy returned live room JSON and a countable `players/maxPlayers` shape. Browser fallback loaded local `index.html`, mocked `/api/drawing-world-rooms`, confirmed exactly two secondary cards, no old Dice/Mammon links, `드로잉월드 온라인` count updating to `8/50`, and zero page/console errors. `node tools/smoke-index-entry.cjs`, `node tools/smoke-singularity-race-v01-lock.cjs`, `npm run check:singularity-race`, `git diff --check`, and full `npm run check` passed.
Blocked: Local static server cannot execute the Vercel API function, so browser verification mocked that endpoint; deployed Vercel should serve the same-origin API function.
Next: Commit/push/deploy this launcher cleanup when the human wants it public on `simulacra-world.vercel.app`.
Do not: Put Baegeum City v1 or Dice City v1 back into the main visible launcher cards.
...
Changed: Added `src/restored/games/singularity-race-finish-window.js` with an initial shared finish-window formula. This was later superseded on 2026-06-03 by the fixed 30 second finish-window rule. Updated `workers/singularity-race-worker.js` so the first finisher starts a persisted `finishWindowEndsAtMs`, sets a Durable Object alarm, snapshots remaining time and finished/player counts, and ends the race on finish-window expiry. Updated `singularity-race.html` and `singularity-race-track.js` with a `track-finish-window` HUD countdown, server snapshot sync, local-preview matching timer, and DNF result metadata for unfinished runners. Updated Cloudflare/plan docs and smoke guards.
...
Next: Human-play a public/admin room: finish with one runner, confirm the fixed 30 second countdown appears, and confirm DNF/final results after expiry.
Do not: Revert the Worker to `players.every(finishedAtMs)` only, remove the finish-window alarm, reintroduce all-finished immediate finalization, or mark DNF runners as finished in the result list.
