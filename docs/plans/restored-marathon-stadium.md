...
- Movement: `WASD` is now interpreted as a screen-space intent vector and projected onto the current trail segment. On a right-facing straight, `D` advances and `W/S` adjust the lane; on an upward segment, `W` advances and `A/D` adjust the lane. Mobile joystick input does not emulate `S`/reverse anymore; it publishes race intent as `forward` plus `lateral`, so pushing upward always means forward race progress and left/right always means lane movement even on curved or vertical route sections. Held mobile input is resent through the connected input budget instead of relying only on key-change events. Single-axis `W/S` input gets a small diagonal-uphill assist so maze connectors do not feel slow or drift into the side wall when the player is clearly trying to climb. `Shift` sprints, and idle/recover frames restore stamina.
- Items: the 0.1 race item layer is intentionally small. Three rows of five larger item boxes sit on the course, one item slot is allowed, and boxes respawn after a short delay. Box pickup uses a tight visible-contact radius plus a short previous-to-current movement trace owned by `src/restored/games/singularity-race-item-contract.js`, so merely passing near a box should not collect it while actual fast crossings still do. Pickup then opens a short bottom-center item roulette before the final item enters the slot. The roulette is positioned above mobile controls so it does not hide the joystick or action buttons, and track/roulette/mobile item displays now share code-built pixel icons instead of single-letter markers. The current items are Booster, Banana, Stun Shot, and Ink Cloud; Booster is capped to a mild speed multiplier and shows a local boost glow so it does not feel dead, Banana applies a short trap slow, and projectile items use a two-step flow: the first `E` or mobile skill press auto-locks an aim target, mouse click or mobile touch on the track can retarget, and the second `E` or mobile skill press fires. Stun Shot applies short slow/stun effects to local bot runners, and Ink Cloud drops a brief blind/slow hazard with a screen mask when it hits the local runner. On mobile, the visible skill button uses this item slot first.
- Obstacles: each selectable Singularity Race map now has several static course obstacles from `src/restored/games/singularity-race-obstacle-contract.js`. They are not economy/world-editor objects and do not create a shared engine system. Obstacles render in the race/map-preview track effects layer, use the same `progress + laneOffsetPx` lane coordinate as item boxes, and collision pushes a runner slightly backward and sideways while stamping `collisionAtMs` for the bump animation/SFX. Local player movement, local bot movement, connected client prediction, and the Cloudflare Worker all consume the same obstacle collision helper so map hitboxes do not reintroduce client/server rubber-banding.
...
- Basic attack: mouse click on the track or the mobile `공격` button performs the same short-range attack. Basic attack keeps held WASD, Shift, and mobile joystick intent alive, so pressing attack should no longer feel like the controls were dropped. The attack has a cooldown, a one-shot lunge/sweep visual, and a hit-only recoil/burst effect on the target, plus a small server action-lock value for future authority checks. Before the gate opens, basic attack can only stun another runner for positioning practice and cannot reduce HP. After the race starts, hits can stun and deal limited damage; down/respawn still belongs to the combat contract and must become server-owned before public online.
...
Current render budget guard: the hot 60 ms local preview loop uses `renderActionPreviewFrame()` instead of full-page `renderAll()`. Track and runner positions stay hot, while action HUD, standings, and queue slots are throttled; chat messages, skin cards, channel tabs, and debug rails are not redrawn every movement tick. Existing runner DOM nodes must be updated in place rather than re-appended every frame, and the race minimap caches its static SVG and moves only the player dot. The ordinary start-line NPC crowd is a static CSS-only decoration grouped behind the basic-map start area in the back-left spectator corner, with waving signs, and must not be rebuilt by the hot loop or use race skin avatars. Course obstacles are static effect-layer nodes from the obstacle contract; the hot loop may refresh their positions with the rest of track effects, but it must not rebuild runner DOM or move obstacle authority into ad hoc client-only code. `tools/smoke-singularity-race-render-budget.cjs` and `tools/smoke-singularity-race-obstacles.cjs` guard this so future UI work does not accidentally reintroduce full DOM redraws or client/server collision drift into the movement loop.
...
Player start-authority guard: public Cloudflare play still keeps the user page as the normal player entry, while `singularity-race-admin.html?online=cloudflare` may act as an `ADMIN_TOKEN`-authenticated minimal Worker console. The first player who joins the fixed public room remains the temporary room host and may press `경기 시작`, which sends only `start_request`; the Worker decides whether to start the server-owned 10-second countdown. Other players may show `방장 시작 대기중`, chat, skin change, and map preview, but they must not expose countdown start or call host-start from ready/map-preview buttons. Public `operator=1` stays removed, and public admin start/reset/map/open/close must go through Worker `/admin/*` endpoints rather than localStorage/BroadcastChannel. Kick, bot control, and observer-console authority remain future authenticated admin work.
...
- Host model: the first joined public player temporarily owns the visible user-page `start_request` button for this v2 loop. The Worker still validates host ownership, lobby phase, and population before countdown. Authenticated host/admin/operator tooling remains future work.
