...
Player start-authority guard: public Cloudflare play keeps the user page as the normal player entry, while `singularity-race-admin.html?online=cloudflare` acts as an `ADMIN_TOKEN`-authenticated minimal Worker console. The fixed public room starts with entry closed; admin `/admin/open` lets players enter the queue/start rail, then authenticated `/admin/start` begins the server-owned 10-second countdown. Players may show `관리자 대기중` before entry opens and `관리자 시작 대기중` after joining, but they must not expose `경기 시작`, send `start_request`, or receive first-player host authority. Public `operator=1` stays removed, and public admin start/reset/map/open/close must go through Worker `/admin/*` endpoints rather than localStorage/BroadcastChannel. Kick, bot control, and observer-console authority remain future authenticated admin work.
