...
-> countdown
-> move with WASD or the mobile joystick at one slightly faster base pace
-> collect item boxes, use utility items with E or the mobile item button, and use mouse/mobile attacks without dropping held movement
...
- Basic attack: mouse click on the track or the mobile `공격` button performs the same short-range attack. Basic attack keeps held WASD and mobile joystick intent alive, so pressing attack should no longer feel like the controls were dropped. The attack has a cooldown, a one-shot punch/kick visual, a 4.8-progress local/Worker range, and a hit-only recoil/burst effect on the target, plus a small server action-lock value for future authority checks. Local/dev rehearsal can still exercise damage/down/respawn through the combat contract, but public Cloudflare basic attacks are currently impact/stun only: the Worker owns the hit test from current runner positions, applies cooldown plus `stunnedUntilMs`/`slowUntilMs`, and snapshots that state without trusting client origin or reducing HP.
- Sword item: the old sword-swing visual is no longer the base attack by default. It now lives behind `item:sword`: five direct `race-sword-pickup` drops sit on the track, proximity pickup adds the sword to the two-slot item queue, and the next basic attack consumes one sword charge to trigger `useSwordItem()` with a distinct `is-sword-attacking` slash motion and a wider local melee hit. `E` and the mobile item button intentionally skip sword so players do not need a separate item-use input for the sword. Public Worker HP authority for sword remains a separate future slice.
- Finish window: the first finisher starts a server/client-shared 30 second finish window instead of requiring every player to reach 100%. This applies even if the first finisher is currently the only player, so the end-of-race countdown is always visible before finalization. During the window the HUD shows a `track-finish-window` countdown plus finished/total count; when it expires, unfinished runners are DNF and results sort finishers by finish time before unfinished runners by progress.
...
- Balance target: attacks are for mid-pack chaos and clutch defense, not the main way to win. The fastest line should usually be clean movement, item timing, and checkpoint routing.
...
- Single trail geometry: `src/restored/games/marathon-trail-geometry.js`, with smoothing helper `src/restored/games/marathon-trail-smoothing.js` and wall-interior artifact clipping in `src/restored/games/marathon-trail-wall-clip.js`
...
The action-race layer uses WASD/mobile-joystick movement with no player sprint toggle, checkpoint-reward `E` skill use, and mouse/mobile-directed attack. `src/restored/games/marathon-input-contract.js` normalizes movement inputs into server-checkable frames and keeps Shift-held movement on the base `run`/`push` pace. The base profile skin has no `E` skill; the current D/C/B/A/S checkpoint reward grade owns the temporary skill pool and charges until the next checkpoint replaces it. Basic attack now preserves held movement locally; balance comes from cooldown, hit range, stun timing, and later server-owned action-lock validation rather than forcibly clearing the player's controls.
...
In public Cloudflare mode, the admin page can trigger the same pre-race narration through authenticated `/admin/narration`, which broadcasts a `narration_start` packet to connected player pages after in-game entry is open. This server packet reuses the client overlay only and must remain presentation-only.
...
- Mobile constraints: the live race surface should use joystick movement without a separate sprint button; older pace-control buttons stay limited to the non-live marathon panel.
...
- Spectator joins use their own participant type. Spectators can enter during countdown/racing, view the active runner snapshots, and chat in the shared room/spectator/notice surfaces, but must not publish movement, attack, skill, ready, or host-control packets. Spectator and post-finish watch cameras share `watchTargetRunnerId`, with previous/next runner controls cycling only unfinished runners.
...
- Movement authority note: the Worker must use the same trail tangent/normal movement projection, base run progress speed, and obstacle collision helper as the client prediction layer. Raw direction shortcuts such as "any input means forward", "direction.y means lane", faster Worker-only progress constants, Shift-only speed branches, or client-only obstacle hitboxes are not allowed because they make server snapshots fight the player's local prediction on curves and collisions, producing automatic drift and rubber-band stutter.
...
- Spectators: allowed online participant type, can enter during countdown/racing when host policy allows it, do not consume the 30 runner slots, can chat, and can watch player snapshots without becoming controllable runners. The player page keeps their chat action visible and lets them cycle the shared unfinished-runner watch camera.
...
8. Add WASD/mobile joystick/E/mouse input, checkpoint character reward, skills, attack, and respawn contracts.
...
- Manual play notes: confirm 30 runners, WASD/mobile joystick/E/mouse intent, checkpoint character assignment, attack stall, checkpoint respawn, finish ranking, and no online lobby while unavailable.
