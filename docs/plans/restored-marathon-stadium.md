...
- Items: the 0.1 race item layer is intentionally small. Three rows of five larger item boxes sit on the course, one item slot is allowed, and boxes respawn after a short delay. Box pickup uses a wider progress/lane radius plus a short previous-to-current movement trace owned by `src/restored/games/singularity-race-item-contract.js`, so runners do not visually pass through a box without collecting it during frame skips, booster movement, or connected prediction correction. Pickup then opens a short bottom-center item roulette before the final item enters the slot. The roulette is positioned above mobile controls so it does not hide the joystick or action buttons. The current items are Booster, Banana, and Stun Shot; Booster is capped to a mild speed multiplier so it does not reopen wall-collision instability, while Banana and Stun Shot apply short slow/stun effects to local bot runners. On mobile, the visible skill button uses this item slot first.
...
Current audio guard: Singularity Race uses feature-owned BGM manifest ids, not raw desktop paths. `audio:bgm:singularity-race:squid-wake` is queued as a one-time personal entry track after the player's first entry gesture. `audio:bgm:singularity-race:dont-stop-me` starts when the race gate opens, and `audio:bgm:singularity-race:tjie-she-pen` starts after the primary race track ends and loops while the race is still active. `Drowning (Drowning)` remains unused for a later map version. Playback stays client-local and must unlock through a real click/tap/key gesture because browser autoplay policy can block silent automatic starts. `tools/smoke-singularity-race-audio.cjs` guards manifest registration, runtime audio wiring, the hidden audio root, and the unused Drowning boundary.
