...
Player start-authority guard: public Cloudflare play uses the user page only. The first player who joins the fixed public room becomes the temporary room host and may press `경기 시작`, which sends only `start_request`; the Worker decides whether to start the server-owned 10-second countdown. Other players may show `방장 시작 대기중`, chat, skin change, and map preview, but they must not expose countdown start or call host-start from ready/map-preview buttons. Public `operator=1` and public admin start/reset controls are disabled for this v2 loop; authenticated public host roles remain future work.
