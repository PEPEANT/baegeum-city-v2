...
- Public room in-game entry starts closed by default. Players may still join the public waiting queue while `entryOpen:false`; the admin console uses `/admin/open` to move waiting players into the in-game start rail.
...
- During countdown/racing, new player and spectator joins are blocked with `room_join_closed`. `entryOpen:false` must not block lobby/queue joins; it only blocks admin start and in-game start-rail entry. Mid-race spectator entry is intentionally deferred until the simpler public loop is stable.
