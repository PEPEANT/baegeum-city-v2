...
- User-room host end is allowed only after `resultSnapshot` exists. `/rooms/host/end` closes the room, clears the host token, keeps the result snapshot briefly for save/share, and then lets the auto-delete TTL remove the room from the active registry.
