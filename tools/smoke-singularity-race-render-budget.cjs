...
const trackEffectsBody = functionBody("renderTrackEffects");
const itemRouletteBody = functionBody("renderItemRoulette");
...
assert(pageSource.includes("connectedRemoteSnapshots: new Map()"), "connected remote runner interpolation buffer should exist");
assert(pageSource.includes("function getLocalRunner()"), "local player lookup helper should exist");
assert(pageSource.includes("function setLocalRunner("), "local player mutation helper should exist");
assert(pageSource.includes("function getRemoteRunnerEntries()"), "remote runner lookup should not depend on array order");
assert(!pageSource.includes("state.runners[0]"), "hot/local paths must not assume local runner is array index 0");
assert(!pageSource.includes("state.runners.slice(1)"), "target selection must not assume remote runners start at index 1");
assert(pageSource.includes("syncConnectedRemoteInterpolationBuffer(packet, state.runners)"), "server snapshots should feed remote interpolation before rendering");
assert(pageSource.includes("resolveConnectedRemoteVisualRunner(runner, visualTiming.now)"), "remote runners should render through interpolation");
assert(pageSource.includes("connectedRemoteInterpolationDelayMs"), "remote interpolation should use a bounded delay");
assert(pageSource.includes("CLOUDFLARE_PING_PROBE_INTERVAL_MS"), "Cloudflare live ping probe interval should exist");
assert(pageSource.includes('type: "ping"'), "Cloudflare ping probe should send a raw ping packet");
assert(pageSource.includes('packet.type === "pong"'), "Cloudflare pong packets should update live ping metrics");
assert(pageSource.includes("handleCloudflarePongPacket"), "Cloudflare pong handler should exist");
assert(pageSource.includes("livePingSample.rttMs"), "netcode HUD should render live RTT when connected");
...
assert(pageSource.includes("race-save-marker"), "race screen should expose save checkpoint markers on the track");
assert(trackEffectsBody.includes("createRaceSavePointEffectEntries()"), "track effects should include keyed save checkpoint entries");
assert(pageSource.includes("key: `save:${index + 1}`"), "save checkpoint markers should use stable keyed DOM entries");
assert(pageSource.includes("is-cleared") && pageSource.includes("is-next"), "save checkpoint markers should show cleared and next states");
assert(pageSource.includes("renderRaceStandings"), "race screen should expose live standings");
assert(pageSource.includes("runner-ride-vehicle"), "runner avatars should expose a visible turbo-car riding layer");
...
assert(pageSource.includes("decorateRaceMessageWithRank"), "race chat should decorate player messages with live rank labels");
assert(pageSource.includes("resolveChatMessageRunnerId"), "race chat rank labels should resolve sender ids to runner ids");
...
assert(trackBody.includes("!attacking && !swordAttacking && !stunned && motion.running"), "attack, sword, and stun states should suppress run animation");
assert(trackBody.includes("!attacking && !swordAttacking && !stunned && runner.collisionAtMs"), "stun, sword, and attack states should take visual priority over collision flash");
assert(pageSource.includes("race-sword-pickup"), "race screen should expose direct sword pickups on the track");
assert(pageSource.includes("runner-sword-slash"), "sword item should render a distinct slash motion");
...
assert(trackEffectsBody.includes("syncKeyedChildren(elements.trackEffects"), "track effects should reuse keyed DOM nodes instead of rebuilding the layer each frame");
assert(!trackEffectsBody.includes("elements.trackEffects.replaceChildren(...nodes)"), "track effects must not replace the whole effect layer during hot movement");
assert(itemRouletteBody.includes("ensureRaceItemRoulettePanel"), "item roulette should keep its panel node while spinning");
assert(!itemRouletteBody.includes("replaceChildren(createRaceItemRouletteNode"), "item roulette must not recreate the panel every frame");
assert(pageSource.includes("orderLocalRunnerFirst([...merged.runners])"), "connected snapshots should keep the local runner at index 0 for legacy local-player code");
assert(pageSource.includes('participant.participantId === "runner:you"'), "localized Cloudflare snapshots should find the local participant by runner:you");
