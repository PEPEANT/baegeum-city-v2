...
function assertPlayerAudioWiring(byId) {
  ...
  assert(pageSource.includes("syncEntryGateMusicCue"), "player page should cue Squid wake when the entry gate opens");
  assert(pageSource.includes("requestEntryGateMusic(\"entry_gate_open\")"), "entry gate opening should play the Squid wake entry cue");
