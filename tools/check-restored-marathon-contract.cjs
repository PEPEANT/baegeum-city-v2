...
function assertMarathonDocs() {
  ...
  const runnerViewSource = read(runnerViewPath), trackSource = read(trackPath), queueSource = read(queuePath), localSimSource = read(localSimPath), predictionSource = read(predictionPath), devOnlineSource = read(devOnlinePath), raceControlSource = read(raceControlPath), chatStorageSource = read(chatStoragePath);
...
(async () => {
  [
    contractPath, inputPath, characterSkillPath, rewardGradePath, combatPath, trailGeometryPath, viewPath, raceSkinPath, flowPath, runnerViewPath, trackPath, queuePath, localSimPath, predictionPath, devOnlinePath, raceControlPath,
